const CACHE="commute-dashboard-restore-v38";
...
self.addEventListener("fetch",e=>{
 ...
 if(page){
  e.respondWith(fetch(r,{cache:"no-store"}).then(res=>{const cp=res.clone();caches.open(CACHE).then(c=>c.put(r,cp));return res}).catch(()=>caches.match(r).then(x=>x||caches.match("./v32.html"))));
  return;
 }
